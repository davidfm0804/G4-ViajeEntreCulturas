...
document.querySelector('input[type="submit"]').addEventListener('click', async function(event){
    ...
    if (valid) {
        ...
        formData.append('id', idContinente)

        // Realizar una solicitud fetch para comprobar si el continente ya existe en la base de datos
        try {

            const repiteResponse = await fetch('comprobarModifContinente.php', {
                method: 'POST',
                body: formData
            });

            const repiteData = await repiteResponse.json();
            if (repiteData.existe){
                alert("El nuevo nombre de continente no puede ser igual al anterior.");
                return;
            }

            const continenteResponse = await fetch('comprobarContinente.php', {
                method: 'POST',
                body: formData
            });

              // Verificamos si la respuesta es válida
              if (!continenteResponse.ok) {
                throw new Error('Error en la solicitud: ' + continenteResponse.status);
            }

            // Intentamos obtener la respuesta como JSON
            const data = await continenteResponse.json();  // Parseamos la respuesta JSON
            ...
            const insertResponse = await fetch('modificarContinente.php', {
                method: 'POST',
                body: formData
            });
                ...
                p.textContent = `Continente renombrado: ${nombreContinente}`;
                p.style.margin = '4%';
